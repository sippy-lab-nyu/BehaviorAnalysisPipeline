...
DEFAULT_SETTINGS = {'start_window': 1, 'end_window' : 2,'y_minimum': -200, 'avg_trace': True, 'y_maximum': 200,'tone_length': 0.1 , 'align_1': False, 'align_2': True,'theme': sg.theme()}
# "Map" from the settings dictionary keys to the window's element keys
SETTINGS_KEYS_TO_ELEMENT_KEYS = {'start_window': 'START', 'end_window': 'END' ,'y_minimum': 'YMIN','y_maximum': 'YMAX','tone_length': 'TONELENGTH' ,'avg_trace': 'AVGTRACE', 'align_1':'ALIGN1','align_2':'ALIGN2','theme': '-THEME-'}
...
#Datasplice WINDOW
def create_data_splicer_window(datasplice):
...
def create_settings_window(settings):
    ...
    layout = [  [sg.Text('Visualizer Settings', font='Any 15')],
                ...
                [sg.Checkbox('AverageTrace', default=True, key='AVGTRACE')],
...
def main():
    ...
    while True:             # Event Loop
        ...
        if window is None:
            ...
            left_col = [[sg.Multiline(size=(50,15),key='Output',pad=(0,20))], 
                         [sg.Canvas(key="DayPerformance",border_width=100)]]
            right_col= [[sg.TabGroup(tab_group_layout,
                                   enable_events=True,
                                   key='-TABGROUP-')]]
            ...
            window = sg.Window('SippyLab ModifiedGoNoGo Data Analysis', layout, element_justification='center', size= (1400,850))
        ...
        if event == 'Summary':
            if 'validfile1' in locals():
                ...
                #GraphTrace
                    if 'figure_canvas_agg4' in locals():
                        if figure_canvas_agg4:
                            figure_canvas_agg4.get_tk_widget().forget()
                            plt.close('all')
                            ax4.cla()
                    if str(metadata['training_phase']) == 'Go-NoGo Task':
                        Response = States[States['Id'].isin(['Hit','Miss','FalseAlarm','CorrectRejection','Pull Penalty'])]
                        ResponseNew = Response
                        ResponseNew.index = ResponseNew.index+1
                        HitRate = pd.DataFrame(columns=('X','value'))
                        FalseAlarmRate = pd.DataFrame(columns=('X','value'))
                        TotalGo = pd.DataFrame(columns=('X','value'))
                        TotalNoGo = pd.DataFrame(columns=('X','value'))
                        HitRate.loc[0] = [0,0]
                        FalseAlarmRate.loc[0] = [0,0]
                        TotalGo.loc[0] = [0,0]
                        TotalNoGo.loc[0] = [0,0]
                        for x in range(len(ResponseNew)):
                            if ResponseNew['Id'].iloc[x] == 'Hit':
                                TotalGo.loc[x] = ['line' + str(x),TotalGo['value'].iloc[x-1]+1]
                                HitRate.loc[x] = ['line' + str(x),HitRate['value'].iloc[x-1]+1]
                                TotalNoGo.loc[x] = ['line' + str(x),TotalNoGo['value'].iloc[x-1]]
                                FalseAlarmRate.loc[x] = ['line' + str(x),FalseAlarmRate['value'].iloc[x-1]]
                            elif ResponseNew['Id'].iloc[x] == 'Miss':
                                TotalGo.loc[x] = ['line' + str(x),TotalGo['value'].iloc[x-1]+1]
                                HitRate.loc[x] = ['line' + str(x),HitRate['value'].iloc[x-1]]
                                TotalNoGo.loc[x] = ['line' + str(x),TotalNoGo['value'].iloc[x-1]]
                                FalseAlarmRate.loc[x] = ['line' + str(x),FalseAlarmRate['value'].iloc[x-1]]
                            elif ResponseNew['Id'].iloc[x] == 'FalseAlarm':
                                TotalGo.loc[x] = ['line' + str(x),TotalGo['value'].iloc[x-1]]
                                HitRate.loc[x] = ['line' + str(x),HitRate['value'].iloc[x-1]]
                                TotalNoGo.loc[x] = ['line' + str(x),TotalNoGo['value'].iloc[x-1]+1]
                                FalseAlarmRate.loc[x] = ['line' + str(x),FalseAlarmRate['value'].iloc[x-1]+1]
                            elif ResponseNew['Id'].iloc[x] == 'CorrectRejection':
                                TotalGo.loc[x] = ['line' + str(x),TotalGo['value'].iloc[x-1]]
                                HitRate.loc[x] = ['line' + str(x),HitRate['value'].iloc[x-1]]
                                TotalNoGo.loc[x] = ['line' + str(x),TotalNoGo['value'].iloc[x-1]+1]
                                FalseAlarmRate.loc[x] = ['line' + str(x),FalseAlarmRate['value'].iloc[x-1]]
                            elif ResponseNew['Id'].iloc[x] == 'Pull Penalty':
                                TotalGo.loc[x] = ['line' + str(x),TotalGo['value'].iloc[x-1]+1]
                                HitRate.loc[x] = ['line' + str(x),HitRate['value'].iloc[x-1]]
                                TotalNoGo.loc[x] = ['line' + str(x),TotalNoGo['value'].iloc[x-1]]
                                FalseAlarmRate.loc[x] = ['line' + str(x),FalseAlarmRate['value'].iloc[x-1]]
                                ...
                                TotalNoGo.loc[x] = ['line' + str(x),TotalNoGo['value'].iloc[x-1]]
                                FalseAlarmRate.loc[x] = ['line' + str(x),FalseAlarmRate['value'].iloc[x-1]]
                        TotalGo['value']=TotalGo['value'].mask(TotalGo['value']==0).fillna(1)
                        TotalNoGo['value']=TotalNoGo['value'].mask(TotalNoGo['value']==0).fillna(1)
                        RunHitRate = HitRate['value'].loc[1:len(HitRate)-1]/TotalGo['value'].loc[1:len(TotalGo)-1]
                        RunFalseAlarmRate = FalseAlarmRate['value'].loc[1:len(FalseAlarmRate)-1]/TotalNoGo['value'].loc[1:len(TotalNoGo)-1]
                        #Running Graph
                        fig4 = matplotlib.figure.Figure(figsize=(5, 3.5), dpi=100)
                        ax4 = fig4.add_subplot(111)
                        ax4.plot(RunHitRate.index, RunHitRate, color='green')
                        ax4.plot(RunFalseAlarmRate.index, RunFalseAlarmRate, color='red')
                        figure_canvas_agg4 = FigureCanvasTkAgg(fig4, window['DayPerformance'].TKCanvas)
                        figure_canvas_agg4.draw()
                        figure_canvas_agg4.get_tk_widget().pack(side='left')
                        figure_canvas_agg4
                        ax4.set_xlabel('Trial Number')
                        ax4.set_ylabel('Performance')
                        ax4.set_ylim([0,1])
                    if str(metadata['training_phase']) == 'Single Tone':
                        Response = States[States['Id'].isin(['Hit','Miss','Pull Penalty'])]
                        ResponseNew = Response
                        ResponseNew.index = ResponseNew.index+1
                        HitRate = pd.DataFrame(columns=('X','value'))
                        TotalGo = pd.DataFrame(columns=('X','value'))
        ...
        if event == 'Splice Dataset':
           if 'validfile1' in locals():
                if validfile1 == True:
                    event, values = create_data_splicer_window(settings).read(close=True)
        ...
        if event == 'Save Plots':
            ...
            if mouseplotdestination == None or mouseplotdestination == "":
                ...
                if os.path.exists(mouseplotdestination):
                    ...
                    if 'figure_canvas_agg4' in locals():
                        if figure_canvas_agg4:
                            fig4.savefig(str(mouseplotdestination) +'/runningperformance'+ str(metadata['mousefilename'])+'.png')       
                    else:
